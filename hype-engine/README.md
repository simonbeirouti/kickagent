...
```bash
npm test    # simulation harness: 18 behaviour checks against the scripted replay
...
| `src/highlights.js` | **Hype highlights / clip markers.** When hype crosses ≥75 it opens a timestamp window (with a 10s lead-in), tracks the peak + top topic, and closes on hysteresis (<65) or a 40s cap — re-crossings within 20s extend the same highlight. Each capture gets a headline (biggest kicks gift > sub > topic) and lands in a recap reel; in a real integration the windows map onto KICK VOD timestamps / the clip API. |
...
| `test/sim.js` | Behaviour checks: baseline locks, ramp registers, spam is flagged & doesn't spike the score, suggestion fires in the lull, **Hit Me, Kick Me** measures "up", "poker" tops topics, a trending-gap suggestion names a topic absent from chat, highlight clip markers capture the ramp and bet burst (and never the spam attack), and the same relative burst scores similarly at 1 and 40 msg/s. |
...
highlights.on('highlight', (h) => addToRecap(h));          // clip marker captured
